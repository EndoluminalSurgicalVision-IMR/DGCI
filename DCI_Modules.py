...
class FCBlock(MetaModule):
    ...
    def __init__(self, in_features, out_features, num_hidden_layers, hidden_features,
                 outermost_linear=False, nonlinearity='relu', weight_init=None, first_initial=True, last_initial=True):
        super().__init__()

        self.first_layer_init = None
        nls_and_inits = {'sine30': (Sine(30), lambda m: sine_init(m, 30), first_layer_sine_init, last_layer_sine_init),
                         'sine15': (Sine(15), lambda m: sine_init(m, 15), first_layer_sine_init, last_layer_sine_init),
                         'sine': (Sine(1), lambda m: sine_init(m, 1), first_layer_sine_init, last_layer_sine_init),
                         'relu': (nn.ReLU(inplace=True), init_weights_normal, None, None),
                         'leakyrelu': (
                             nn.LeakyReLU(negative_slope=0.1, inplace=True), init_weights_leakyrelu, None, None),
...
class SingleBVPNet(MetaModule):
    ...
    def __init__(self, out_features=1, type='sine', in_features=2,
                 mode='mlp', hidden_features=256, num_hidden_layers=3, outermost_linear=True, first_initial=True,
                 last_initial=True, **kwargs):
        super().__init__()
        self.mode = mode
        self.net = FCBlock(in_features=in_features, out_features=out_features, num_hidden_layers=num_hidden_layers,
                           hidden_features=hidden_features, outermost_linear=outermost_linear, nonlinearity=type,
...
class SDFBVPNet(SingleBVPNet):
    def forward(self, model_input, params=None):
        coords_org = model_input['coords'].requires_grad_(True)
        coords = coords_org
        output = self.net(coords_org, get_subdict(params, 'net')) + torch.norm(coords_org, dim=-1, keepdim=True)
        return {'model_in': coords_org, 'model_out': output}


def init_weights_normal(m):
    if type(m) == BatchLinear or type(m) == nn.Linear or type(m) == nn.Conv1d:
        if hasattr(m, 'weight'):
